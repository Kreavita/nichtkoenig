...
function fadeInfo(title, content) {
	...
	e.style.opacity=0;
	e.innerHTML=`<div class="alertbox"><div id="title">${title}</div><div id="content">${content}</div><div class="action-row"><div class="superbtn" onclick="fadeRemove(this.parentElement.parentElement.parentElement)">Close</div></div></div>`
...
class Nav {
	...
	join(){
		fadeInfo("Enter the Lobby Code", )
	}
	create(){
		fadeUpdate("main", "jquery", "html/newgame.html");
	}
	account(){
		fadeUpdate("main", "jquery", "html/account.html");
	}
}